...
<section class="blog-sec inner-content-wrapper">
  ...
  <div class="container px-1 px-md-4 mx-auto">
    <div class="card" style="background-color: #ECEFF1;">
      <form id="contactForm" method="POST" action="{{route('trackingFedex')}}" >
        @csrf
      ...
      <div class="row d-flex justify-content-between px-3 top">
        <div class="d-flex flex-column">
            <p class="mb-0">Status: <span  class="font-weight-bold">{{$dataResponse[0]["trackResults"][0]["latestStatusDetail"]["statusByLocale"]??''}}</span></p>
            <p class="mb-0">Location address: <span class="font-weight-bold">{{$dataResponse[0]["trackResults"][0]["recipientInformation"]["address"]["countryName"]??''}} </span></p>
            <p> description: <span class="font-weight-bold">{{$dataResponse[0]["trackResults"][0]["serviceDetail"]["description"]??''}}</span></p>
        </div>
        <div class="d-flex">
          <h5>Fedex <span class="text-primary font-weight-bold">#{{$tracking_number ?? ''}}</span></h5>
        </div>
      </div>
      ...
      <div class="row d-flex justify-content-center">
          ...
          <ul id="progressbar" class="text-center">
              <li class="{{ in_array($dataResponse[0]["trackResults"][0]["latestStatusDetail"]["statusByLocale"], ['Delivered', 'In transit', 'pre-transit']) ? 'active' : '' }} step0"></li>
              <li class="{{ in_array($dataResponse[0]["trackResults"][0]["latestStatusDetail"]["statusByLocale"], ['Delivered', 'In transit', 'pre-transit']) ? 'active' : '' }} step0"></li>
              <li class="{{ in_array($dataResponse[0]["trackResults"][0]["latestStatusDetail"]["statusByLocale"], ['Delivered', 'In transit']) ? 'active' : '' }} step0"></li>
              <li class="{{ in_array($dataResponse[0]["trackResults"][0]["latestStatusDetail"]["statusByLocale"], ['Delivered']) ? 'active' : '' }} step0"></li>
          </ul>
          </div>
      </div>
      <div class="row top">
      ...
      <div class="row top"  id="allUpdates" style="display: none;">
        @foreach ($dataResponse[0]["trackResults"] as $event )
          @php
            $date = new DateTime($event['dateAndTimes'][0]['dateTime']);
          ...
          <div class="row d-flex icon-content">
            ...
            <div class="col-lg-6 col-md-5 d-flex flex-column">
              <p class="font-weight-bold">
                {{$event["serviceDetail"]["description"]??''}}<br>
                {{$event["recipientInformation"]["address"]["countryName"]??''}} {{$event["recipientInformation"]["address"]["countryCode"]??''}}<br>
                {{$event["latestStatusDetail"]["statusByLocale"]??''}}
              </p>
            </div>
          </div>
        @endforeach
